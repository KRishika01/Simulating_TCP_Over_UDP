...
void handling_incoming_chunks(struct sockaddr_in *recv_addr,int recv_fileD) {
    ...
    while(1) {
        int b_read = recvfrom(recv_fileD,&packet_recv,sizeof(packet_recv),0,(struct sockaddr *)recv_addr,&recv_len);
        if(b_read < 0) {
        ...
        if(b_read > 0) {
            ...
            if(counter_for_ack % 3 != 0) {
                if(sendto(recv_fileD,&ack_recv,sizeof(ack_recv),0,(struct sockaddr *)recv_addr,recv_len) < 0) {
...
int main() {
    ...
    if(bind(fileD, (struct sockaddr *)&server_addr,sizeof(server_addr)) < 0) {
    ...
    while(1) {
        ...
        else if(flag_send_receive == false) {
            printf("------RECEIVING------\n");
            handling_incoming_chunks(&client_addr,fileD);
